...
!function (window) {
    ...
    window.define = function (name, deps, callback) {
        ...
        defines.push({ name: name, args: [deps, callback] });

        window.require(deps, function () {
            callback.apply(window, Array.prototype.slice.apply(arguments));
        });
    }
    ...
    window.require = function (deps, callback) {
        setTimeout(function () {
            // Make all defines are loaded
            defines.forEach(function (define, index) {
                oldDefine.apply(window, define.args);
